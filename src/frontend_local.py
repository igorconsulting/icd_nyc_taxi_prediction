...
from src.inference import load_batch_of_features_from_store, load_predictions_from_store
...
with st.spinner(text='Fetching model predictions from the store'):
    predictions_df = _load_predictions_from_store(
        from_pickup_hour=current_date - timedelta(hours=1), to_pickup_hour=current_date
    )
...
prev_hour_predictions_ready = (
    False
    if predictions_df[
        predictions_df.pickup_hour == (current_date - timedelta(hours=1))
    ].empty
    else True
)
...
if next_hour_predictions_ready:
    ...
    raise Exception(
        'Features are not available for the last 2 hours. Is your feature \
...
with st.spinner(text='Generating NYC Map'):
    INITIAL_VIEW_STATE = pdk.ViewState(
        latitude=40.7831, longitude=-73.9712, zoom=11, max_zoom=16, pitch=45, bearing=0
    )
    ...
    r = pdk.Deck(
        layers=[geojson], initial_view_state=INITIAL_VIEW_STATE, tooltip=tooltip
    )
