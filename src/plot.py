...
def plot_one_sample(
    ...
    title = (
        f'Pick up hour={features_["pickup_hour"]}, location_id={features_["pickup_location_id"]}'
        if display_title
        else None
    )
    fig = px.line(
...
def plot_ts(ts_data: pd.DataFrame, locations: Optional[List[int]] = None):
    ...
    """
    ts_data_to_plot = (
        ts_data[ts_data.pickup_location_id.isin(locations)] if locations else ts_data
    )
